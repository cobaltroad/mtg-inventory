<script lang="ts">
	...
	interface Props {
		...
		onMoveToInventory?: (item: InventoryItem) => void;
	}

	let { items, loading = false, onItemsChange, onMoveToInventory }: Props = $props();
	...
	async function handleQuantityUpdate(item: InventoryItem, newQuantity: number) {
		...
		try {
			const response = await fetch(`${base}/api/wishlist/${item.id}`, {
				method: 'PATCH',
	...
	async function handleRemove(item: InventoryItem) {
		...
		try {
			...
			toast = { message: `Removed ${item.card_name} from wishlist`, type: 'success' };
		} catch (err) {
			// Rollback on failure
					...
					<tr>
						...
						<td>
							<div class="actions-cell">
								...
								>
									<ArrowRight size={18} />
									<span class="move-text">To Inventory</span>
								</button>
								<button
									class="remove-btn"
...
<style>
	...
	:global(.dark) .wishlist-table {
		background: #1f2937;
	}

	:global(.dark) .wishlist-table thead {
		background: #374151;
		border-bottom-color: #4b5563;
	}

	:global(.dark) .wishlist-table th {
		color: #e5e7eb;
	}

	:global(.dark) .wishlist-table tbody tr {
		border-bottom-color: #374151;
	}

	:global(.dark) .wishlist-table tbody tr:hover {
		background: #374151;
	}
